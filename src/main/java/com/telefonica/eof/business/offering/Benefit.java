...
public class Benefit {
	...
	if ("null".equalsIgnoreCase(offersBenefitsRequestDto.getCurrentOffering())
	...
	discountList.forEach(discountOffer -> {
	    ...
	    List<PriceProperties> discountDetail = pricePropertiesRepository
		    .getDiscountPriceDetail(discountOffer.getBenefitBillingOfferCid());
	    String valueAbp = discountDetail.stream().filter(x -> x.getNamePropAbp().equals("Discount value")).map(p -> p.getValueAbp())
		    .collect(Collectors.joining());
	    String valueAbpType = discountDetail.stream().filter(x -> x.getNamePropAbp().equals("Discount type")).map(p -> p.getValueAbp())
		    .collect(Collectors.joining());
    ...
    // public List<OfferingType> getBenefitDiscount(OffersBenefitsRequestDto
    // offersBenefitsRequestDto, String amdocsCatalogItemID,
    ...
    // discountParamsDto.setDownloadSpeed(amdocsDownloadSpeed);
    //
    // List<WirelineServiceBenefits> discountList =
    // wirelineServiceBenefitsRepository.getDiscount(discountParamsDto);
    //
    ...
    // discountList.forEach(discountOffer -> {
    // String nameComp =
    // componentsMasterRepository.getComponentName(discountOffer.getBenefitsComponentCid());
    // List<PriceProperties> discountDetail =
    // pricePropertiesRepository.getDiscountPriceDetail(discountOffer.getBenefitBillingOfferCid());
    ...
    // String spsName =
    // relationMasterRepository.getDiscountSpsName(discountOffer.getBenefitThemePackSpsCid());
    // String nameBo =
    // billingOfferMasterRepository.getBillingOfferName(discountOffer.getBenefitBillingOfferCid()).getNameBo();
    //
